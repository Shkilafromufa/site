...
switch ($method) {
    case 'POST':
        ...
        $stmt = $db->prepare('INSERT INTO certificates (title, description, image_path, alt_text) VALUES (?,?,?,?)');
        $stmt->execute([
            trim($data['title'] ?? ''),
    ...
    case 'PATCH':
        ...
        $data = json_decode(file_get_contents('php://input'), true) ?? [];
        $stmt = $db->prepare('UPDATE certificates SET title=?, description=?, image_path=?, alt_text=? WHERE id=?');
        $stmt->execute([
            trim($data['title'] ?? ''),
            trim($data['description'] ?? ''),
            trim($data['image_path'] ?? ''),
